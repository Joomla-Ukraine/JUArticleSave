...
class EMT_Tret
{
	 ...
	 */
	const BASE64_PARAGRAPH_TAG = 'cA===';
	const BASE64_BREAKLINE_TAG = 'YnIgLw===';
	const BASE64_NOBR_OTAG     = 'bm9icg===';
	const BASE64_NOBR_CTAG     = 'L25vYnI==';
	...
	public $classes = array(); // p
	public $settings = array(); // br / (с пробелом и слэшем)
	protected $_text = ''; // nobr
	private $disabled = array(); // /nobr
	...
	private function apply_rule($rule)
	{
			...
			else
			{
				if(preg_match('/^[a-z_0-9]+$/i', $rule[ 'function' ]))
				{
					if(method_exists($this, $rule[ 'function' ]))
					{
						...
						$this->_text = preg_replace_callback($rule[ 'pattern' ], array(
							$this,
							$rule[ 'function' ]
						), $this->_text);
		...
		foreach($pattern as $patt)
		{
			...
			if(strpos($preg_arr[ count($preg_arr) - 1 ], 'e') !== false) // eval система
			{
				...
				do
				{
					$this->_text = preg_replace_callback($patt, array(
						$this,
						'thereplcallback'
					), $this->_text);
	...
	protected function preg_replace_e($pattern, $replacement, $text)
	{
		...
		return preg_replace_callback($patt, array(
			$this,
			'thereplcallback'
		), $text);
	}
...
class EMT_Tret_Abbr extends EMT_Tret
{
	...
	public $rules = array(
		...
		'nobr_sm_im'              => array(
			...
			'pattern'     => '/(\s|^|\>|\()(см|им)\.(\040|\t)*([а-яёєїі0-9a-z]+)(\s|\.|\,|\?|\!|$)/iu',
			'replacement' => '\1\2.&nbsp;\4\5'
		),
		'nobr_locations'          => array(
			'description' => 'Расстановка пробелов в сокращениях г., ул., пер., д.',
			'pattern'     => array(
				'/(\s|^|\>)(г|ул|пер|просп|пл|бул|наб|пр|ш|туп)\.(\040|\t)*([а-яёєїі0-9a-z]+)(\s|\.|\,|\?|\!|$)/iu',
				'/(\s|^|\>)(б\-р|пр\-кт)(\040|\t)*([а-яёєїі0-9a-z]+)(\s|\.|\,|\?|\!|$)/iu',
		...
		'nbsp_org_abbr'           => array(
			...
			'pattern'     => '/([^a-zA-Zа-яёєїіА-ЯЁЄЇІ]|^)(ООО|ЗАО|ОАО|НИИ|ПБОЮЛ) ([a-zA-Zа-яёєїіА-ЯЁЄЇІ]|\"|\&laquo\;|\&bdquo\;|<)/u',
			'replacement' => '\1\2&nbsp;\3'
		),
		'nobr_gost'               => array(
...
class EMT_Tret_Dash extends EMT_Tret
{
	...
	public $rules = array(
		...
		'mdash'                      => array(
			...
			'pattern'     => array(
				'/([a-zа-яёєїі0-9]+|\,|\:|\)|\&(ra|ld)quo\;|\|\"|\>)(\040|\t)(—|\-|\&mdash\;)(\s|$|\<)/ui',
				'/(\,|\:|\)|\")(—|\-|\&mdash\;)(\s|$|\<)/ui',
			),
			'replacement' => array(
				'\1&nbsp;&mdash;\5',
		...
		'ka_de_kas'                  => array(
			...
			'disabled'    => true,
			'pattern'     => array(
				'/(\s|^|\&nbsp\;|\>)([а-яёєїі]+)(\040|\t|\&nbsp\;)(ка)([\.\,\!\?\;]|\040|\&nbsp\;|$)/uie',
				'/(\s|^|\&nbsp\;|\>)([а-яёєїі]+)(\040|\t|\&nbsp\;)(де)([\.\,\!\?\;]|\040|\&nbsp\;|$)/uie',
				'/(\s|^|\&nbsp\;|\>)([а-яёєїі]+)(\040|\t|\&nbsp\;)(кась)([\.\,\!\?\;]|\040|\&nbsp\;|$)/uie',
			),
			'replacement' => '($m[1] == "&nbsp;" ? " " : $m[1]) . $m[2]."-".$m[4] . ($m[5] == "&nbsp;"? " " : $m[5])'
		),
	);
...
class EMT_Tret_Date extends EMT_Tret
{
	...
	public $rules = array(
		'years'                        => array(
			...
			'pattern'     => '/(с|по|период|середины|начала|начало|конца|конец|половины|в|между|\([cс]\)|\&copy\;)(\s+|\&nbsp\;)([\d]{4})(-|\&mdash\;|\&minus\;)([\d]{4})(( |\&nbsp\;)?(г\.г\.|гг\.|гг|г\.|г)([^а-яёєїіa-z]))?/eui',
		...
		'space_posle_goda'             => array(
			...
			'pattern'     => '/(^|\040|\&nbsp\;)([0-9]{3,4})(год([ауе]|ом)?)([^a-zа-яёєїі]|$)/ui',
			'replacement' => '\1\2 \3\5'
		),
		'nbsp_posle_goda_abbr'         => array(
			'description' => 'Пробел после года',
			'pattern'     => '/(^|\040|\&nbsp\;|\"|\&laquo\;)([0-9]{3,4})[ ]?(г\.)([^a-zа-яёєїі]|$)/ui',
			'replacement' => '\1\2&nbsp;\3\4'
		),
	);
}
...
class EMT_Tret_Etc extends EMT_Tret
{
	...
	public $rules = array(
		...
		'word_sup'               => array(
			...
			'pattern'     => '/((\s|\&nbsp\;|^)+)\^([a-zа-яёєїі0-9\.\:\,\-]+)(\s|\&nbsp\;|$|\.$)/ieu',
	...
	protected function remove_nbsp()
	{
		...
		$match = '/(^|[^a-zа-яёєїі])([a-zа-яёєїі]+)\&nbsp\;(' . $b . ')/iu';
		...
		$match = '/(' . $e . ')\&nbsp\;([a-zа-яёєїі]+)($|[^a-zа-яёєїі])/iu';
...
class EMT_Tret_Nobr extends EMT_Tret
{
	...
	public $rules = array(
		'super_nbsp'                   => array(
			...
			'pattern'     => '/(\s|^|\&(la|bd)quo\;|\>|\(|\&mdash\;\&nbsp\;)([a-zа-яёєїі]{1,2}\s+)([a-zа-яёєїі]{1,2}\s+)?([a-zа-яёєїі0-9\-]{2,}|[0-9])/ieu',
		...
		'nbsp_in_the_end'              => array(
			...
			'pattern'     => '/([a-zа-яёєїі0-9\-]{3,}) ([a-zа-яёєїі]{1,2})\.( [A-ZА-ЯЁЄЇІ]|$)/u',
			'replacement' => '\1&nbsp;\2.\3'
		),
		'phone_builder'                => array(
		...
		'dots_for_surname_abbr'        => array(
			...
			'pattern'     =>
				array(
					'/(\s|^|\.|\,|\;|\:|\?|\!|\&nbsp\;)([А-ЯЁЄЇІ])\.?(\s|\&nbsp\;)?([А-ЯЁЄЇІ])(\s|\&nbsp\;)([А-ЯЁЄЇІ][а-яёєїі]+)(\s|$|\.|\,|\;|\:|\?|\!|\&nbsp\;)/ue',
					'/(\s|^|\.|\,|\;|\:|\?|\!|\&nbsp\;)([А-ЯЁЄЇІ][а-яёєїі]+)(\s|\&nbsp\;)([А-ЯЁЄЇІ])\.?(\s|\&nbsp\;)?([А-ЯЁЄЇІ])\.?(\s|$|\.|\,|\;|\:|\?|\!|\&nbsp\;)/ue',
				),
			'replacement' =>
				array(
		...
		'hyphen_nowrap_in_small_words' => array(
			...
			'disabled'    => true,
			'cycled'      => true,
			'pattern'     => '/(\&nbsp\;|\s|\>|^)([a-zа-яёєїі]{1}\-[a-zа-яёєїі]{4}|[a-zа-яёєїі]{2}\-[a-zа-яёєїі]{3}|[a-zа-яёєїі]{3}\-[a-zа-яёєїі]{2}|[a-zа-яёєїі]{4}\-[a-zа-яёєїі]{1}|когда\-то|кое\-как|кой\-кого|вс[её]\-таки|[а-яёєїі]+\-(кась|ка|де))(\s|\.|\,|\!|\?|\&nbsp\;|\&hellip\;|$)/uie',
		...
		'hyphen_nowrap'                => array(
			...
			'disabled'    => true,
			'cycled'      => true,
			'pattern'     => '/(\&nbsp\;|\s|\>|^)([a-zа-яёєїі]+)((\-([a-zа-яёєїі]+)){1,2})(\s|\.|\,|\!|\?|\&nbsp\;|\&hellip\;|$)/uie',
...
class EMT_Tret_Number extends EMT_Tret
{
	...
	public $rules = array(
		...
		'numeric_sub'                    => array(
			...
			'pattern'     => '/([a-zа-яёєїі0-9])\_([\d]{1,3})([^@а-яёєїіa-z0-9]|$)/ieu',
			'replacement' => '$m[1] . $this->tag($this->tag($m[2],"small"),"sub") . $m[3]'
		),
		'numeric_sup'                    => array(
			'description' => 'Верхний индекс',
			'pattern'     => '/([a-zа-яёєїі0-9])\^([\d]{1,3})([^а-яёєїіa-z0-9]|$)/ieu',
		...
		'thinsp_between_sect_and_number' => array(
			...
			'pattern'     => '/(§|\&sect\;)(\s|&nbsp;)*(\d+|[IVX]+|[a-zа-яёєїі]+)/ui',
...
class EMT_Tret_OptAlign extends EMT_Tret
{
	...
	public $rules = array(
		'oa_oquote'        => array(
			...
			//'disabled'      => true,
			'pattern'     => array(
				'/([a-zа-яёєїі\-]{3,})(\040|\&nbsp\;|\t)(\&laquo\;)/uie',
				'/(\n|\r|^)(\&laquo\;)/ei'
			),
			'replacement' => array(
...
class EMT_Tret_Punctmark extends EMT_Tret
{
	...
	public $rules = array(
		'auto_comma'                   => array(
			...
			'pattern'     => '/([a-zа-яёєїі])(\s|&nbsp;)(но|а)(\s|&nbsp;)/iu',
			'replacement' => '\1,\2\3\4'
		),
		...
		'fix_brackets_space'           => array(
			...
			'pattern'     => '/([a-zа-яёєїі])(\()/iu',
			'replacement' => '\1 \2'
		),
		'dot_on_end'                   => array(
			'description' => 'Точка в конце текста, если её там нет',
			'disabled'    => true,
			'pattern'     => '/([a-zа-яёєїі0-9])(\040|\t|\&nbsp\;)*$/ui',
			//'pattern' 		=> '/(([^\.\!\?])|(&(ra|ld)quo;))$/',
			'replacement' => '\1.'
		),
	);
...
class EMT_Tret_Quote extends EMT_Tret
{
	...
	public $rules = array(
		...
		'close_quote'       => array(
			...
			'pattern'     => '/([a-zа-яёєїі0-9]|\.|\&hellip\;|\!|\?|\>|\)|\:|\+|\%|\@|\#|\$|\*)((\"|\\\")+)(\.|\&hellip\;|\;|\:|\?|\!|\,|\s|\)|\<\/|\<|$)/uie',
		...
		'close_quote_adv'   => array(
			...
			'pattern'     =>
				array(
					'/([a-zа-яёєїі0-9]|\.|\&hellip\;|\!|\?|\>|\)|\:|\+|\%|\@|\#|\$|\*)((\"|\\\"|\&laquo\;)+)(\<[^\>]+\>)(\.|\&hellip\;|\;|\:|\?|\!|\,|\)|\<\/|$| )/uie',
					'/([a-zа-яёєїі0-9]|\.|\&hellip\;|\!|\?|\>|\)|\:|\+|\%|\@|\#|\$|\*)(\s+)((\"|\\\")+)(\s+)(\.|\&hellip\;|\;|\:|\?|\!|\,|\)|\<\/|$| )/uie',
					'/\>(\&laquo\;)\.($|\s|\<)/ui',
					'/\>(\&laquo\;),($|\s|\<|\S)/ui',
					'/\>(\&laquo\;):($|\s|\<|\S)/ui',
					'/\>(\&laquo\;);($|\s|\<|\S)/ui',
	...
	protected function build_sub_quotations()
	{
		...
		foreach($texts_in as $textx)
		{
			...
			while(true)
			{
				$p = EMT_Lib::strpos_ex($textx, array(
					'&laquo;',
					'&raquo;'
				), $off);
...
class EMT_Tret_Space extends EMT_Tret
{
	...
	public $rules = array(
		'nobr_twosym_abbr'                     => array(
			...
			'pattern'     => '/([a-zA-Zа-яёєїіА-ЯЁЄЇІ])(\040|\t)+([A-ZА-ЯЁЄЇІ]{2})([\s\;\.\?\!\:\(\"]|\&(ra|ld)quo\;|$)/u',
			'replacement' => '\1&nbsp;\3\4'
		),
		...
		'autospace_after_comma'                => array(
			...
			'pattern'     => array(
				'/(\040|\t|\&nbsp\;)\,([а-яёєїіa-z0-9])/iu',
				'/([^0-9])\,([а-яёєїіa-z0-9])/iu',
			),
			'replacement' => array(
				', \2',
				'\1, \2'
		...
		'autospace_after_pmarks'               => array(
			...
			'pattern'     => '/(\040|\t|\&nbsp\;|^|\n)([a-zа-яёєїі0-9]+)(\040|\t|\&nbsp\;)?(\:|\)|\,|\&hellip\;|(?:\!|\?)+)([а-яёєїіa-z])/iu',
			'replacement' => '\1\2\4 \5'
		),
		'autospace_after_dot'                  => array(
			'description' => 'Пробел после точки',
			'pattern'     => array(
				'/(\040|\t|\&nbsp\;|^)([a-zа-яёєїі0-9]+)(\040|\t|\&nbsp\;)?\.([а-яёєїіa-z]{5,})($|[^a-zа-яёєїі])/iue',
				'/(\040|\t|\&nbsp\;|^)([a-zа-яёєїі0-9]+)\.([а-яёєїіa-z]{1,4})($|[^a-zа-яёєїі])/iue',
			),
			'replacement' => array(
				//'\1\2. \4',
				'$m[1].$m[2]."." .( $m[5] == "." ? "" : " ").$m[4].$m[5]',
		...
		'nbsp_before_open_quote'               => array(
			...
			'pattern'     => '/(^|\040|\t|>)([a-zа-яёєїі]{1,2})\040(\&laquo\;|\&bdquo\;)/u',
			'replacement' => '\1\2&nbsp;\3'
		),
		...
		'no_space_posle_hellip' => array(
			...
			'pattern'     => '/(\&laquo\;|\&bdquo\;)( |\&nbsp\;)?\&hellip\;( |\&nbsp\;)?([a-zа-яёєїі])/ui',
...
class EMT_Tret_Symbol extends EMT_Tret
{
	...
	public $rules = array(
		...
		'apostrophe'     => array(
			...
			'pattern'     => '/(\s|^|\>|\&rsquo\;)([a-zа-яёєїі]{1,})\'([a-zа-яёєїі]+)/ui',
...
class EMT_Tret_Text extends EMT_Tret
{
	...
	public $rules = array(
		...
		'no_repeat_words' => array(
			...
			'pattern'     => array(
				'/([а-яёєїі]{3,})( |\t|\&nbsp\;)\1/iu',
				'/(\s|\&nbsp\;|^|\.|\!|\?)(([А-ЯЁЄЇІ])([а-яёєїі]{2,}))( |\t|\&nbsp\;)(([а-яёєїі])\4)/eu',
			),
			'replacement' => array(
				'\1',
...
class EMT_Base
{
	 ...
	 * @param bool  $altname - альтернативное имя, если хотим например иметь два одинаоковых терта в обработке
	 *
	 * @return bool
	 *
	 * @since 3.5
...
class EMTypograph extends EMT_Base
{
	...
	protected $all_options = array(
		...
		'Space.bracket_fix'              => array(
			...
			'selector'    => array(
				'Space.nbsp_before_open_quote',
				'Punctmark.fix_brackets'
			)
		),
